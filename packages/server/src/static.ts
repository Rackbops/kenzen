...
/**
 * A small static-file handler for the SPA. Simplified from `Rackbops/artifact-console`'s
 * `packages/host/src/static.ts` (design.md section 3) for K4-2's scope: serves from an
 * absolute `staticDir`, rejects path traversal, returns 404 for a genuinely missing known-type
 * asset, and falls back to `index.html` for extension-less routes so client-side routing works.
 * Dropped for now: artifact-console's content-hashed-bundle cache-control classification --
 * there's no real Vite/rollup build output to classify until K4-7/K4-8 build the real SPA;
 * revisit then.
 */
...
export function spaHandler(staticDir: string): (c: Context) => Promise<Response> {
  ...
  return async (c: Context): Promise<Response> => {
    ...
    if (requested !== null) {
      ...
      // extension-less path (including a route like /repos/kenzen) is a client route -> fall
